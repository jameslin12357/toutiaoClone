...
$(window).scroll(function() {
    if (active){
        if($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
            ...
            var topicid = window.location.pathname.slice(window.location.pathname.match(/\d/)["index"],window.location.pathname.lastIndexOf("/"));

            $.ajax({
                type: "get",
                url: `/topicfollowers?topicid=${topicid}&offset=${offset}`,
